...
export default function WritingPage() {
  const perimeter = useParams();
   ...
   const {data: posts} = useQuery({
    ...
    const filtered = posts?.filter((post: any) => post.category_id === perimeter.id);
        ...
        <div className='lg:space-y-1 lg:p-3'>
          {filtered?.map((post: any, idx: number) => {
            if(post.id === '' || post.id === undefined) {
              ...
              return (
                <PostListItems key={idx} slug={post.title} index={idx}  date={post.createdAt} description={post.short_description} />
              )
            }
          })}
      </div>
...
const PreviewPost = ({post}: {post:any}) => {
     ...
     useEffect(() => {
      if (activeId !== null) {
        const matchingData = post?.find((post: any) => parseInt(post.preview_id) === activeIndex);
              ...
              <Detail.ContentContainer>
                <Detail.Header>
                  <Detail.Title>
                    ...
                    <span className='text-tertiary inline-block leading-snug'>{formatDate(activePost.createdAt)}</span>
                </Detail.Header>
                        <div className=''>
                            ...
                            ]}>{activePost.content.toString()}</Markdown>
                        </div>  
              </Detail.ContentContainer>
          </Detail.Container>
      ) : (
...
const PostListItems = ({slug, index, date, description}: {slug: string, date: string, index: number, description: string}) => {
            ...
            : 'text-gray-1000 dark:text-gray-100'
              ...
              activeId === index
                ...
                : 'text-gray-1000 text-opacity-40 dark:text-white dark:text-opacity-60'
                  ...
                  <div className=''>
                    <Link href={`/${slug}`}>
                        <Button icon={<FaBeer width={5} height={5} color={'#000'} />}  label='view' className='default' size='sm' raduis='sm' variant='primary'  />
                    </Link>
                  </div>
        </div>
      </LayoutGroup>
